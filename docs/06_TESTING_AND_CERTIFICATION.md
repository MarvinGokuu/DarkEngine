...
6. **Shutdown Elegante**: `GracefulShutdownTest`. Verifica que todos los buffers y la memoria Off-Heap liberen limpiamente en menos de 1 segundo sin llamadas de OS (kill signal).
   - *Nota sobre Artefactos de Pruebas (Audio Underflow)*: Debido a que la suite de pruebas utiliza agresivamente `Runtime.getRuntime().halt(0)` para asegurar cierres limpios sin deadlocks de JVM entre tests, el driver de audio (`audiodg.exe`) puede reportar un pico de CPU o "Underflow". La auditoría arquitectónica (v4.6.3) documenta que este es un falso positivo inofensivo derivado exclusivamente de la velocidad a la que se aniquilan los contextos en el entorno de pruebas automatizado, y no ocurre en un flujo de producción estándar.
