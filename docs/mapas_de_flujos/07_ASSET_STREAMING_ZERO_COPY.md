...
graph TD
    ...
    subgraph cpu_async [Hilo Secundario: DarkAssetStreamer]
        A -.->|Memory Mapped File| B[Memoria Virtual OS]
        B -->|Mapeo FFM| C[(MemorySegment Off-Heap)]
...
## Leyenda Técnica:
*   **Memory Mapped File (MMAP):** Técnica del kernel del Sistema Operativo que proyecta el disco duro directamente sobre la RAM, permitiendo acceso a velocidad nativa sin copiar bytes de un buffer a otro.
