...
graph TD
    ...
    subgraph fase3 [Fase 3: Post-Procesado y Escalado]
        ...
        E2 --> G{"DarkFSRSystem<br/>Compute Shader"}
        G -->|EASU + RCAS| H(("Presentation FBO 4K"))
    end
    
    %% Prevención de Fugas (Phase 4.5.0)
    subgraph shutdown [Graceful Shutdown]
        I(Kernel Poison Pill) -.->|glDeleteBuffers / Textures| B
        I -.->|Destruye Descriptores| C1
    end
    %% Estilos AAA
    ...
    classDef poison fill:#c0392b,stroke:#e74c3c,stroke-width:2px,color:#fff;
...
*   **VRAM Leak Eradication:** El Kernel Poison Pill fuerza el llamado a `destroy()` sobre la abstracción RHI, asegurando que todos los descriptores (SSBOs, Texturas) se borren limpiamente sin fugas en el Driver.
