...
- **`DarkClusteredSystem.dispatchGrid()`**: Eliminado `new float[16]` (heap) + `Arena.ofConfined()`. Reemplazado por `SCRATCH_INV_PROJ` (campo estÃ¡tico) + `DarkRenderScratchpad`.
...
- **SIMD Vector API (Furia MatemÃ¡tica)**:
...
- **G-Buffer Infrastructure (Fase 27 â€” Mission A)**:
  ...
  - **Normal Buffer**: `GL_RGBA16F` texture (`GL_COLOR_ATTACHMENT1`) â€” stores geometric surface vectors for lighting passes.
  - FBO completeness verified via `glCheckFramebufferStatus` â€” fails fast if VRAM allocation fails.
...
- **Missing Wiring Resolved**:
  ...
  - Injected into `DarkEngineWindow.initNativeWindow()` in correct order:
    `OpenGL FFI â†’ GPU Culling init â†’ Deferred Pipeline init`.
...
- **Asynchronous Metrics Communication**:
  - [EngineStateChannel.java](file:///C:/Users/usuario/Documents/GitHub/SovereignEngine/src/sv/dark/ui/EngineStateChannel.java) implementing a thread-safe ring-buffered metrics channel from the kernel to the GUI.
...
- **Metrics HTTP Server Port & Memory Leak**:
  - Bound `DarkMetricsServer` to a static reference in [AdminController.java](file:///C:/Users/usuario/Documents/GitHub/SovereignEngine/src/sv/dark/admin/AdminController.java) and added `stopControlPlane()` to properly close the server, free the networking port, and teardown background threads.
  - Linked `AdminController.stopControlPlane()` to the kernel shutdown hooks in [EngineKernel.java](file:///C:/Users/usuario/Documents/GitHub/SovereignEngine/src/sv/dark/kernel/EngineKernel.java).
- **Event Dispatcher Deadlock Risks**:
  - In [DarkEventLane.java](file:///C:/Users/usuario/Documents/GitHub/SovereignEngine/src/sv/dark/bus/DarkEventLane.java), added interruption checks to `BLOCK` backpressure spin loops to prevent thread lock-ups during shutdown.
  - In [DarkRingBus.java](file:///C:/Users/usuario/Documents/GitHub/SovereignEngine/src/sv/dark/bus/DarkRingBus.java), implemented a volatile `closed` lifecycle flag that immediately rejects updates with an `IllegalStateException` on shutdown.
- **Simulated Particle Determinism**:
  - Moved the random number generator `RNG` in [DarkParticleSystem.java](file:///C:/Users/usuario/Documents/GitHub/SovereignEngine/src/sv/dark/core/DarkParticleSystem.java) from a static class field to a non-static instance field, ensuring multi-instance test runs are 100% deterministic and isolated.
